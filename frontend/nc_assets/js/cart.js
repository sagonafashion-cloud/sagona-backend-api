...
import { escapeHtml, encodeJsArg } from './config.js';
...
function renderCart() {
  ...
  container.innerHTML = cart.map(item => {
    ...
    return `
      <div class="table-row">
        <img src="${escapeHtml(item.image || '')}" style="width:60px;height:60px;object-fit:cover"
        ...
        <button onclick="openTryOnModal(decodeURIComponent('${encodeJsArg(item.id)}'),decodeURIComponent('${encodeJsArg(item.image)}'),decodeURIComponent('${encodeJsArg(item.name)}'))"
